...
The version release is automated with a gitlab-ci pipeline. It is separated into two stages.

> **Important:** Assure that you are on branch "latest" if you create or publish a version.

### Stage 1 - Create a new draft version
- Checkout the main branch and run:
  - `npm run createNewVersion <NEW-VERSION-NUMBER>`
- This command automatically
  - creates an archive branch for the "old" version
  - updates the version in package.json with `<NEW-VERSION-NUMBER>`
  - updates the version in gitlab-ci.yml with `<NEW-VERSION-NUMBER>`
  - executes 2 git pushes:
    1. Push - Init new version
       - Mark new version as `hidden` to exclude it from version-dropdown
       - Mark new version as `draft` to enable redirects
    2. Push - Reset `UPDATE_ALIAS_DRAFT` to `false`
       - The draft redirect process is initialized in the first push. The second push resets the `gitlab-ci.yml` file.

The draft version is now accessible at url path `/draft/`.

### Stage 2 - Publish the draft version
If you wish to publish the draft and include it in the version-dropdown, you need to run:
- `npm run publishVersion`

This will mark the version as `latest` and set `hidden` to `false`.

### Delete a version
Run `git fetch origin publish:publish` so the publish-branch is up to date.

To remove a version from the branch `publish` install dependencies and run:
- `mike delete 0.0.X -b publish --push`

> **HINT** This will remove the version from branch `publish`. It will not remove the version branch.
