...
In der InGrid-Umgebung dient pyCSW als zentrales Katalog-Ziel für einliefernde Komponenten (Editor, Harvester) und stellt Metadaten via CSW-Schnittstelle für nutzende Komponenten wie das Portal bereit.
...
## Einliefernde Komponenten

### InGrid Editor

Der InGrid Editor kann Metadatensätze via CSW-T direkt in pyCSW einliefern. Die CSW-T-Schnittstelle des Editors ist unter [Editor API: CSW-T]({{ fix_url('guides/editor-api-cswt.md') }}) dokumentiert.

Damit der Editor gegen pyCSW publizieren kann, müssen folgende Voraussetzungen erfüllt sein:
...
``` bash
curl --location 'http://<pycsw-host>/csw' \
...
Der InGrid Harvester kann pyCSW als **CSW-Server Katalogziel** verwenden. Dabei werden geerntete Metadaten per CSW-T an pyCSW übertragen.

Einrichtung in der Harvester-Konfiguration:

1. In der Harvester-Oberfläche eine neue Katalog-Verbindung vom Typ **CSW** anlegen
2. Als Endpunkt den pyCSW-Endpoint eintragen: `http://<pycsw-host>:<port>/csw`
...
## Abfragende Komponenten

### Portal

Das InGrid Portal ruft Metadaten für die Darstellung über die **InGrid API** ab, die auf Elasticsearch zugreift. Alternativ kann pyCSW als externe CSW-Quelle direkt eingebunden werden.
...
Die CSW-Schnittstelle von pyCSW unterstützt Anfragen per **GET** und **POST** (XML). Die folgenden Beispiele zeigen gängige Abfragemuster.

!!! info "Ausgabeformat"
    Alle Beispiele verwenden **ISO 19139** (`http://www.isotc211.org/2005/gmd`) als `outputSchema`. Dieses Format wird für den Austausch von Geodaten-Metadaten im InGrid-Kontext vorausgesetzt.
...
Alle Dokumente können paginiert abgerufen werden. Mit `startPosition` wird der Cursor verschoben, mit `maxRecords` die Seitengröße festgelegt. Der Antwortparameter `numberOfRecordsMatched` liefert die Gesamttrefferanzahl.
...
### Abfrage nach ResourceIdentifier
